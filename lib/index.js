...
function parseAttributes(attributes) {
  ...
  for (var key in attributes) {
    ...
    if (key[0] === '{' && key[1] === '{' && key[key.length - 2] === '}' && key[key.length - 1] === '}') {
      var attrsId = 0;
      var attrsKey = '__derby_attrs_expr__';
      while (attributesMap[attrsKey + attrsId]) ++attrsId;
      attributesMap[attrsKey + attrsId] =
        new templates.AttributesExpression(createExpression(key.substr(2, key.length - 4), true));
      continue;
    }
