...
function createAttributesValue(attributes) {
  ...
  return (isLiteral) ? literalAttributes : new expressions.ObjectExpression(dynamicAttributes);
}
...
function parseArrayElement(element, name, viewAttributes) {
  ...
  if (viewAttribute instanceof expressions.ArrayExpression) {
    if (item instanceof expressions.Expression) {
      viewAttribute.items.push(item);
    } else {
      var expression = new expressions.LiteralExpression(item);
    ...
    viewAttributes[camelName] = (item instanceof expressions.Expression) ?
      new expressions.ArrayExpression([item]) : [item];
