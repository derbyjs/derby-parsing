...
controller.model = {
  data: {
    key: 'green'
  , lightTemplate: parsing.createTemplate('light {{_page.colors[key].name}}')
  , _page: {
      colors: {
        green: {
          name: 'Green'
...
describe('Expression::dependencies', function() {
  ...
  describe('brackets', function() {
    ...
    it('gets bracket + bracket + path dependencies', function() {
      var expression = create('_page.colors[_page.key][_page.variation].hex');
      expect(expression.dependencies(context)).to.eql([
        ['_page', 'key'],
        ['_page', 'variation'],
    ...
    it('gets nested bracket + path dependencies', function() {
      var expression = create('_page.colors[_page.keys[_page.index]].name');
      expect(expression.dependencies(context)).to.eql([
        ['_page', 'index'],
        ['_page', 'keys', 1],
        ['_page', 'colors', 'green', 'name']
  ...
  describe('operators', function() {
    ...
    it('gets chained operator dependencies', function() {
      var expression = create('_page.nums[0] + (_page.nums[3] - _page.nums[2])');
      expect(expression.dependencies(context)).to.eql([
        ['_page', 'nums', 0, '*'],
        ['_page', 'nums', 3, '*'],
        ['_page', 'nums', 2, '*']
    ...
    it('gets bracket + bracket dependencies', function() {
      var expression = create('_page.nums[_page.first] + _page.nums[_page.second]');
      expect(expression.dependencies(context)).to.eql([
        ['_page', 'first'],
        ['_page', 'nums', 2, '*'],
        ['_page', 'second'],
    ...
    it('gets operator inside bracket dependencies', function() {
      var expression = create('_page.keys[_page.nums[2] - _page.nums[0]]');
      expect(expression.dependencies(context)).to.eql([
        ['_page', 'nums', 2, '*'],
        ['_page', 'nums', 0, '*'],
        ['_page', 'keys', 1],
    ...
    it('gets path + literal + path dependencies', function() {
      var expression = create('_page.nums[0] + (100 - _page.nums[2])');
      expect(expression.dependencies(context)).to.eql([
        ['_page', 'nums', 0, '*'],
        ['_page', 'nums', 2, '*']
      ]);
    ...
    it('gets path + bracket dependencies', function() {
      var expression = create('_page.nums[2] + _page.nums[_page.second]');
      expect(expression.dependencies(context)).to.eql([
        ['_page', 'nums', 2, '*'],
        ['_page', 'second'],
        ['_page', 'nums', 3, '*']
    ...
    it('gets path + literal inside bracket dependencies', function() {
      var expression = create('_page.keys[_page.nums[2] - 2]');
      expect(expression.dependencies(context)).to.eql([
        ['_page', 'nums', 2, '*'],
        ['_page', 'keys', 1]
      ]);
  ...
  describe('relative paths', function() {
    describe('with block', function() {
      it('gets expression alias dependencies', function() {
        var aliasExpression = parsing.createExpression('with _page.colors.green');
      ...
      it('gets subpath from expression alias dependencies', function() {
        var aliasExpression = parsing.createExpression('with _page.colors.green');
        var blockContext = context.child(aliasExpression);
        var expression = create('this.name');
        expect(expression.dependencies(blockContext)).to.eql([
          ['_page', 'colors', 'green', 'name']
        ]);
      ...
      it('gets function template alias dependencies', function() {
        var aliasExpression = parsing.createExpression('with passThrough(_page.colors.green)');
        var blockContext = context.child(aliasExpression);
        var expression = create('this');
        expect(expression.dependencies(blockContext)).to.eql([
          ['_page', 'colors', 'green', '*']
      ...
      it('gets subpath from function template alias dependencies', function() {
        var aliasExpression = parsing.createExpression('with passThrough(_page.colors.green)');
        var blockContext = context.child(aliasExpression);
        var expression = create('this.name');
        expect(expression.dependencies(blockContext)).to.eql([
      ...
      it('gets template in model alias dependencies', function() {
        var aliasExpression = parsing.createExpression('with lightTemplate');
        var blockContext = context.child(aliasExpression);
        var expression = create('this');
        expect(expression.dependencies(blockContext)).to.eql([
          ['key'],
      ...
      it('gets subpath from template in model alias dependencies', function() {
        ...
        var aliasExpression = parsing.createExpression('with lightTemplate');
        var blockContext = context.child(aliasExpression);
        var expression = create('this.length');
        expect(expression.dependencies(blockContext)).to.eql([
          ['key'],
          ['_page', 'colors', 'green', 'name'],
  ...
  describe('aliases', function() {
    describe('with block', function() {
      it('gets expression alias dependencies', function() {
        var aliasExpression = parsing.createExpression('with _page.colors.green as #color');
        var blockContext = context.child(aliasExpression);
        var expression = create('#color');
        expect(expression.dependencies(blockContext)).to.eql([
          ['_page', 'colors', 'green']
      ...
      it('gets function template alias dependencies', function() {
        var aliasExpression = parsing.createExpression('with passThrough(_page.colors.green) as #color');
        var blockContext = context.child(aliasExpression);
        var expression = create('#color');
        expect(expression.dependencies(blockContext)).to.eql([
          ['_page', 'colors', 'green', '*']
      ...
      it('gets template in model alias dependencies', function() {
        var aliasExpression = parsing.createExpression('with lightTemplate as #color');
        var blockContext = context.child(aliasExpression);
        var expression = create('#color');
        expect(expression.dependencies(blockContext)).to.eql([
          ['key'],
      ...
      it('gets subpath from template in model alias dependencies', function() {
        ...
        var aliasExpression = parsing.createExpression('with lightTemplate as #color');
        var blockContext = context.child(aliasExpression);
        var expression = create('#color.length');
        expect(expression.dependencies(blockContext)).to.eql([
          ['key'],
          ['_page', 'colors', 'green', 'name'],
          ['lightTemplate', 'length']
    ...
    describe('each block', function() {
      it('gets item alias dependencies', function() {
        var aliasExpression = parsing.createExpression('each _page.keys as #key, #index');
        var eachContext = context.eachChild(aliasExpression, 0);
        var expression = create('#key');
      ...
      it('gets subpath from item alias dependencies', function() {
        var aliasExpression = parsing.createExpression('each _page.keys as #key, #index');
        var eachContext = context.eachChild(aliasExpression, 0);
        var expression = create('#key.length');
        expect(expression.dependencies(eachContext)).to.eql([
  ...
  describe('view attributes', function() {
    describe('expression values', function() {
      it('gets path dependencies', function() {
        var attributes = {
          color: create('_page.colors.green')
        };
      ...
      it('gets function dependencies', function() {
        var attributes = {
          color: create('passThrough(_page.colors.green)')
        };
        var viewContext = context.viewChild(view, attributes);
        var expression = create('@color');
        expect(expression.dependencies(viewContext)).to.eql([
    ...
    describe('template values', function() {
      ...
      it('gets function template attribute dependencies', function() {
        var attributes = {
          color: parsing.createTemplate('{{passThrough(_page.colors.green)}}')
        };
        var viewContext = context.viewChild(view, attributes);
        var expression = create('@color');
        expect(expression.dependencies(viewContext)).to.eql([
